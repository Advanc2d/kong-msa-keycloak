  ...
  <div class="board-list">
    ...
    <table class="w3-table-all">
      ...
      <tr v-for="(row, idx) in list" :key="idx">
        <td >{{ row.idx }}</td>
        <td style="padding-left: 20px; text-align: left !important;"><a v-on:click="fnView(`${row.idx}`)" class="title-link">{{ row.title }}</a></td>
        <td >{{ row.author }}</td>
        <td >{{ row.createdAt }}</td>
      </tr>
      </tbody>
...
export default {
  ...
  methods: {
    fnGetList() {
      ...
      // Spring boot로 만든 Backend Server로 params에 값을 담아서 보내서 결과 값 res를 받음
